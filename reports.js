...
async function renderHtmlToPDF(contentHTML, filename) {
  ...
  try {
    // Give browser layout/images a moment to finish before capture.
    ...
    // Keep a small footer area available for page numbers.
    ...
    for (let page = 1; page <= totalPages; page++) {
      ...
      // Page number is added directly, so no placeholder/cover-up is needed.
      pdf.setFontSize(8);
    ...
    // Browser download only. No DriveApp/fetch/upload is used here.
...
function buildReportHeaderHTML(title, subtitle) {
  ...
  return `
    <div dir="${escapeHtml(L.dir)}" lang="${escapeHtml(L.htmlLang)}"
      ...
      <div style="display:flex;align-items:center;justify-content:space-between;
        ...
        <div style="text-align:center;flex:1;min-width:0;">
          ...
          <div style="font-size:13px;color:#667eea;font-weight:600;margin-top:3px;">
            ${escapeHtml((L.academicYearLabel || 'Academic Year') + ': ' + getAcademicYear())}
          </div>
        </div>
  ...
  Object.keys(byClass).sort().forEach(cls => {
    ...
    html += `
      <h3 style="color:#667eea;font-size:15px;margin:18px 0 8px;
        ...
        🏫 ${escapeHtml(L.classLabel)} ${escapeHtml(cls)}
      </h3>
    `;
  ...
  try {
    ...
    Object.keys(byClass).sort().forEach(cls => {
      ...
      const absRow = [1, 2, 3, 4, 5, 6].map(n => {
        ...
        if (lecture) return L.noneAbsent;
        return '';
      });
    ...
    await renderHtmlToPDF(
      body,
      `Daily_Report_${sanitizeFileName(fDate, 'Date')}.pdf`
    );
  } catch (error) {
...
async function exportWeeklyPDF() {
  try {
    ...
    const subtitle =
      `${L.weekLabel} ${week.number}: ${formatDate(week.start)} - ${formatDate(week.end)}`;
    ...
    await renderHtmlToPDF(
      body,
      `Weekly_Report_${sanitizeFileName(week.number, 'Week')}.pdf`
    );
  } catch (error) {
    handlePDFExportError(error);
  }
...
async function exportRosterPDF() {
  try {
    ...
    const min = parseInt(document.getElementById('minAbsences').value, 10) || 0;
  ...
  try {
    ...
    body += `
      <div style="margin-top:50px;display:flex;justify-content:space-between;
        gap:40px;page-break-inside:avoid;break-inside:avoid;">
